...
import com.accenture.accounts.clients.CustomerFeignClient;
...
public class AccountService implements IAccountService {
    ...
    @Override
    public AccountsWithCustomerInformationDTO fetchCustomerInformationWithAccountsByAccount(Long accountNumber) {
            Account account = repository.findByAccountNumber(accountNumber);
            ResponseEntity<CustomerDTO> customerResponse = customerFeignClient.fetchCustomerInformation(account.getCustomerId());
            CustomerDTO customerPayload = customerResponse.getBody();
            AccountsWithCustomerInformationDTO accountsWithCustomerInformationDTO = AccountMapper.mapCustomerInformationAccountsToDto(customerPayload, new AccountsWithCustomerInformationDTO());
            accountsWithCustomerInformationDTO.setAccountNumber(accountNumber);
